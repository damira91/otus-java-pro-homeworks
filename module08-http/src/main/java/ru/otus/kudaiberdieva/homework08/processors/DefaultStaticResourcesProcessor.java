...
public class DefaultStaticResourcesProcessor implements RequestProcessor, RequestProcessorHeaderType {
    @Override
    public void execute(HttpRequest httpRequest, OutputStream output) throws IOException {
        String filename = httpRequest.getUri().substring(1);
        Path filePath = Paths.get("module08-http/static/", filename);
